...
from form import TaskForm, LoginForm, RegistrationForm  ### form.py로 빼놨으니까
...
# Application 요청 처리 전에 관리자 계정 생성
@app.before_request
...
def add_task():
    ...
    if form.validate_on_submit():
        ...
        new_task = Task(
            title=title, contents=contents, input_date=input_date, due_date=due_date, user_id=session["user_id"]
        )
...
def delete_task(task_id):
    ...
    task = Task.query.get_or_404(task_id)
    if task.user_id != session["user_id"]:
        flash("You ar not authorized to delete this task.", "danger")
        return redirect(url_for('index'))
...
if __name__ == '__main__':
    # with app.app_context():         ## DB에 acess할 수 있는 권한 주는.. ==> app.before_request에서 다 설정
    #     db.create_all()
    app.run(debug=True)
