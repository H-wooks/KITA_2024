...
def admin_analysis():
    ...
    for user in users:
        ...
        tasks_ongoing = total_tasks - tasks_on_time - tasks_overdue
...
@app.route("/generate_plot_data", methods=["GET"])
def generate_plot_data():
    ...
    if not user_analysis:
        return jsonify({"error": "No user analysis data found"}), 400

    return jsonify(user_analysis)

@app.route("/logout_and_close")
def logout_and_close():
    try:
        session.clear()
        return render_template("logout_and_close.html")
